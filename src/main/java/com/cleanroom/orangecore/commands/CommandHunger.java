package com.cleanroom.orangecore.commands;
...
public class CommandHunger extends CommandBase
{
	...
	public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender commandSender, @Nonnull String[] args) throws CommandException
	{
		if (args.length > 0)
		{
			...
			int maxHunger = OrangeCoreAPI.accessor.getMaxHunger(playerToActOn);
			int newHunger = args.length >= 2 ? parseInt(args[1], 0, maxHunger) : parseInt(args[0], 0, maxHunger);

			OrangeCoreAPI.mutator.setHunger(playerToActOn, newHunger);
